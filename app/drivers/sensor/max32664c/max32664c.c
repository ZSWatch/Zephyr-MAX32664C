...
static uint8_t max32664c_raw_queue_buffer[CONFIG_MAX32664C_QUEUE_SIZE *
                                          sizeof(struct max32664c_raw_t)];
static uint8_t
    max32664c_report_queue_buffer[CONFIG_MAX32664C_QUEUE_SIZE *
                                  (sizeof(struct max32664c_raw_t) +
                                   sizeof(struct max32664c_report_t))];

#if CONFIG_MAX32664C_USE_EXTENDED_REPORTS
static uint8_t
    max32664c_ext_report_queue_buffer[CONFIG_MAX32664C_QUEUE_SIZE *
                                      (sizeof(struct max32664c_raw_t) +
 ...
 *  @param delay    Command delay in milliseconds. See the documentation to get
 * the delay for a specific command. Use MAX32664C_DEFAULT_CMD_DELAY for the
 * default delay.
 *  @return         0 when successful
 */
int max32664c_i2c_transmit(const struct device *dev, uint8_t *tx_buf,
                           uint8_t tx_len, uint8_t *rx_buf, uint32_t rx_len,
                           uint16_t delay_ms) {
 ...
 *              NOTE: See page 10 of the SpO2 and Heart Rate User Guide for
 ...
 *  @return     0 when successful
 */
static int max32664c_start_thread(const struct device *dev) {
  ...
  return k_thread_name_set(data->thread_id, "max32664c_worker");
}
...
static int max32664c_set_mode_raw(const struct device *dev) {
  ...
  k_msgq_alloc_init(&data->raw_queue, sizeof(struct max32664c_raw_t),
                    CONFIG_MAX32664C_QUEUE_SIZE);
#endif /* CONFIG_MAX32664C_USE_STATIC_MEMORY */

  data->op_mode = MAX32664C_OP_MODE_RAW;

  return max32664c_start_thread(dev);
}
...
static int max32664c_set_mode_wake_on_motion(const struct device *dev) {
  ...
  /* Set the motion detection threshold (see Table 12 in the SpO2 and Heart Rate
   * Using Guide)
   */
  /*  Duration: 0.2 s */
  /*  Threshold: 0.5 g */
 ...
 *  @return             0 when successful
 */
static int max32664c_set_mode_algo(const struct device *dev,
                                   enum max32664c_device_mode device_mode,
                                   enum max32664c_algo_mode algo_mode,
                                   bool extended) {
  uint8_t rx;
  uint8_t tx[5];
  struct max32664c_data *data = dev->data;
  ...
  if (device_mode == MAX32664C_OP_MODE_ALGO_AEC) {
    ...
    tx[3] = 0x00;
    tx[4] = 0x64;
    if (max32664c_i2c_transmit(dev, tx, 5, &rx, 1,
                               MAX32664C_DEFAULT_CMD_DELAY)) {
      return -EINVAL;
    }
...
#ifndef CONFIG_MAX32664C_USE_STATIC_MEMORY
  if (!extended) {
    k_msgq_alloc_init(&data->report_queue, sizeof(struct max32664c_report_t),
                      CONFIG_MAX32664C_QUEUE_SIZE);
  }
#if CONFIG_MAX32664C_USE_EXTENDED_REPORTS
  else {
    k_msgq_alloc_init(&data->ext_report_queue,
                      sizeof(struct max32664c_ext_report_t),
                      CONFIG_MAX32664C_QUEUE_SIZE);
  }
#endif /* CONFIG_MAX32664C_USE_EXTENDED_REPORTS */
...
static int max32664c_sample_fetch(const struct device *dev,
  ...
  if ((data->op_mode == MAX32664C_OP_MODE_ALGO_AEC) ||
      (data->op_mode == MAX32664C_OP_MODE_ALGO_AGC)) {
...
static int max32664c_attr_set(const struct device *dev,
  ...
  if ((chan != SENSOR_CHAN_HEARTRATE) &&
      (chan != SENSOR_CHAN_BLOOD_OXYGEN_SATURATION) &&
      (chan != SENSOR_CHAN_RESPIRATION_RATE) &&
      (chan != SENSOR_CHAN_SKIN_CONTACT) && (chan != SENSOR_CHAN_ACCEL_X) &&
...
static int max32664c_init(const struct device *dev) {
  ...
  LOG_DBG("Version: %d.%d.%d", data->hub_ver[0], data->hub_ver[1],
          data->hub_ver[2]);
...
#if CONFIG_MAX32664C_USE_STATIC_MEMORY
  k_msgq_init(&data->raw_queue, max32664c_raw_queue_buffer,
              sizeof(struct max32664c_raw_t),
              sizeof(max32664c_raw_queue_buffer) /
                  sizeof(struct max32664c_raw_t));
  k_msgq_init(&data->report_queue, max32664c_report_queue_buffer,
              sizeof(struct max32664c_report_t),
              sizeof(max32664c_report_queue_buffer) /
                  sizeof(struct max32664c_report_t));

#if CONFIG_MAX32664C_USE_EXTENDED_REPORTS
  k_msgq_init(&data->ext_report_queue, max32664c_ext_report_queue_buffer,
              sizeof(struct max32664c_ext_report_t),
              sizeof(max32664c_ext_report_queue_buffer) /
                  sizeof(struct max32664c_ext_report_t));
#endif /* CONFIG_MAX32664C_USE_EXTENDED_REPORTS */
#endif /* CONFIG_MAX32664C_USE_STATIC_MEMORY */
...
static int max32664c_pm_action(const struct device *dev,
  ...
  case PM_DEVICE_ACTION_TURN_OFF: {
    ...
    tx[1] = 0x00;
    tx[2] = 0x01;
    if (max32664c_i2c_transmit(dev, tx, 3, &rx, 1,
                               MAX32664C_DEFAULT_CMD_DELAY)) {
      return -EINVAL;
    }
    break;
...
#define MAX32664C_INIT(inst)                                                   \
  ...
  static const struct max32664c_config max32664c_config_##inst = {             \
      ...
      .min_integration_time_idx =                                              \
          DT_INST_ENUM_IDX(inst, min_integration_time),                        \
      .min_sampling_rate_idx = DT_INST_ENUM_IDX(inst, min_sampling_rate),      \
      .max_integration_time_idx =                                              \
          DT_INST_ENUM_IDX(inst, max_integration_time),                        \
  ...
  SENSOR_DEVICE_DT_INST_DEFINE(                                                \
      inst, max32664c_init, PM_DEVICE_DT_INST_GET(inst),                       \
      &max32664c_data_##inst, &max32664c_config_##inst, POST_KERNEL,           \
      CONFIG_SENSOR_INIT_PRIORITY, &max32664c_driver_api)
