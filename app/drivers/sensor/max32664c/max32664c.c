...
static uint8_t
	max32664c_raw_queue_buffer[CONFIG_MAX32664C_QUEUE_SIZE * sizeof(struct max32664c_raw_t)];
static uint8_t max32664c_report_queue_buffer[CONFIG_MAX32664C_QUEUE_SIZE *
					     (sizeof(struct max32664c_raw_t) +
					      sizeof(struct max32664c_report_t))];

#if CONFIG_MAX32664C_USE_EXTENDED_REPORTS
static uint8_t max32664c_ext_report_queue_buffer[CONFIG_MAX32664C_QUEUE_SIZE *
						 (sizeof(struct max32664c_raw_t) +
 ...
 *  @param delay    Command delay in milliseconds. See the documentation to get the delay for a
 * specific command. Use MAX32664C_DEFAULT_CMD_DELAY for the default delay.
...
static int max32664c_set_mode_raw(const struct device *dev)
{
	...
	data->threadID = k_thread_create(&data->thread, max32664c_thread_stack,
			      ...
			      enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	if ((chan != SENSOR_CHAN_HEARTRATE) && (chan != SENSOR_CHAN_BLOOD_OXYGEN_SATURATION) &&
	    ...
	    (chan != SENSOR_CHAN_ACCEL_Y) && (chan != SENSOR_CHAN_RED) &&
	    (chan != SENSOR_CHAN_GREEN) && (chan != SENSOR_CHAN_IR) && (chan != SENSOR_CHAN_ALL)) {
		return -EINVAL;
	}
...
#if CONFIG_MAX32664C_USE_STATIC_MEMORY
	k_msgq_init(&data->raw_queue, max32664c_raw_queue_buffer, sizeof(struct max32664c_raw_t),
		    sizeof(max32664c_raw_queue_buffer) / sizeof(struct max32664c_raw_t));
	k_msgq_init(&data->report_queue, max32664c_report_queue_buffer,
		    sizeof(struct max32664c_report_t),
		    sizeof(max32664c_report_queue_buffer) / sizeof(struct max32664c_report_t));
